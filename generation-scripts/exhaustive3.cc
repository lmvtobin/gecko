...
/**
 * Command line utility which exhaustively enumerates all 3-manifolds resulting from grids
 * of a given size, simplifies the resulting triangulations as much as possible, and performs 
...
int main(int argc, char* argv[]) {
    ...
    if (argc < 2) {
        std::cerr << "Usage:\n  ./exhaustive3 gridsize [cores=1] [timeout_mins=60] [-save]\n";
        std::cerr << "Example usage for grid size 6 with default values and not saving:\n  ./exhaustive3 6\n";
        std::cerr << "Example usage for grid size 6, on 10 cores, timing out after 15 minutes of trying to decompose, saving to default location:\n  ./exhaustive3 6 10 15 -save\n";
        std::cerr << "Optional positional arguments must be in order, flags (-save) must appear after including all optional arguments\n";
        return 0;
    }
    ...
    long totalGrids = factorial(n)*std::floor((long double)(factorial(n)+1)/std::exp(1.0)) / 2;
    std::cout << "Total grids (x < o): " << n << "!*!" << n << "/2 = " << totalGrids << "\n";
    int tracker = 0;
    int loops = 0;
    gecko::Grid grid = gecko::firstGrid(n);
    do {
        regina::Triangulation<3> trig = gecko::build3Manifold(grid);
        trig.simplify();
        std::string sig = trig.isoSig<regina::IsoSigDegrees<3,1>>();
        unidentifiedCounts[sig]+=1;
        if (gridMap.contains(sig)) {
            gridMap[sig].push_back(grid);
        } else {
            gridMap[sig] = {grid};
        }
        ++loops;
        if (loops == 10000) {
            ++tracker;
            std::cout << "processed " << tracker << "0000 / " << totalGrids << " ...\n";
            loops = 0;
        }
    ...
    std::cout << "Remaining: " << totalGrids-checkTotal << " / " << totalGrids << "\n\n";
    ...
    if (checkTotal != totalGrids) {
        std::cout << "Error: total count (" << checkTotal << ") does not match the original number of totalGrids given (" << totalGrids << ")\n";
    }

    std::cout << "elapsed: " << secondsSince(start) << " s\n\n";

    if (saveResults) {
        saveToFile(finalCounts,"n"+std::to_string(n));
    }
